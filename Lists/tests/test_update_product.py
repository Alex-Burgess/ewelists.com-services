...
@pytest.fixture
def api_gateway_update_product_event():
...
class TestUpdateProductMain:
    def test_update_product_(self, api_gateway_update_product_event, monkeypatch, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        response = update_product.update_product_main(api_gateway_update_product_event)
        body = json.loads(response['body'])
        assert body['quantity'] == 2, "Update to product quantity was not expected result."

    def test_update_product_with_no_quantity(self, api_gateway_update_product_event, monkeypatch, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        api_gateway_update_product_event['body'] = ''
        response = update_product.update_product_main(api_gateway_update_product_event)
        body = json.loads(response['body'])
        assert body['error'] == "API Event did not contain the quantity in the body.", "Error not as expected."

    def test_update_product_with_not_owner(self, api_gateway_update_product_event, monkeypatch, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        api_gateway_update_product_event['requestContext']['identity']['cognitoAuthenticationProvider'] = "cognito-idp.eu-west-1.amazonaws.com/eu-west-1_vqox9Z8q7,cognito-idp.eu-west-1.amazonaws.com/eu-west-1_vqox9Z8q7:CognitoSignIn:12345678-user-0003-1234-abcdefghijkl"
        response = update_product.update_product_main(api_gateway_update_product_event)
        body = json.loads(response['body'])
        assert body['error'] == "No list exists with this ID.", "Error not as expected."

    def test_add_product_with_no_list_id(self, api_gateway_update_product_event, monkeypatch, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        api_gateway_update_product_event['pathParameters']['id'] = '12345678-list-0002-1234-abcdefghijkl'
        response = update_product.update_product_main(api_gateway_update_product_event)
        body = json.loads(response['body'])
        assert body['error'] == "No list exists with this ID.", "Error not as expected."

    def test_add_product_with_no_product_id(self, api_gateway_update_product_event, monkeypatch, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        api_gateway_update_product_event['pathParameters']['productid'] = ''
        response = update_product.update_product_main(api_gateway_update_product_event)
        body = json.loads(response['body'])
        assert body['error'] == "API Event did not contain a Product ID in the path parameters.", "Error not as expected."


def test_handler(api_gateway_update_product_event, monkeypatch, dynamodb_mock):
    monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
    response = update_product.handler(api_gateway_update_product_event, None)
