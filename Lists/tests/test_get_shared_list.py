    ...
    return {
        ...
        "requestContext": {
            ...
            "identity": {
                ...
                "cognitoAuthenticationProvider": "cognito-idp.eu-west-1.amazonaws.com/eu-west-1_vqox9Z8q7,cognito-idp.eu-west-1.amazonaws.com/eu-west-1_vqox9Z8q7:CognitoSignIn:db9476fd-de77-4977-839f-4f943ff5d684",
...
def dynamodb_mock():
    ...
    items = [
        ...
        {"PK": "USER#db9476fd-de77-4977-839f-4f943ff5d685", "SK": "USER#db9476fd-de77-4977-839f-4f943ff5d685", "email": "test.user3@gmail.com", "name": "Test User3", "userId": "db9476fd-de77-4977-839f-4f943ff5d685"},
...
class TestGetListQuery:
    def test_get_list_query(self, dynamodb_mock):
        ...
        items = get_shared_list.get_list_query('lists-unittest', cognito_user_id, "12345678-abcd-abcd-123456789112")
    ...
    def test_get_list_query_no_table_name(self, dynamodb_mock):
        ...
        with pytest.raises(Exception) as e:
            get_shared_list.get_list_query('lists-unittes', cognito_user_id, "12345678-abcd-abcd-123456789112")
    ...
    def test_get_list_query_for_item_that_does_not_exist(self, dynamodb_mock):
        ...
        with pytest.raises(Exception) as e:
            get_shared_list.get_list_query('lists-unittest', cognito_user_id, "12345678-abcd-abcd-123456789112-diff")
        assert str(e.value) == "No query results for List ID 12345678-abcd-abcd-123456789112-diff and user: db9476fd-de77-4977-839f-4f943ff5d684.", "Exception not as expected."
...
class TestGetSharedListMain:
    ...
    def test_get_shared_list_when_not_shared_with_requestor(self, monkeypatch, api_gateway_get_list_event, dynamodb_mock):
        monkeypatch.setitem(os.environ, 'TABLE_NAME', 'lists-unittest')
        api_gateway_get_list_event['requestContext']['identity']['cognitoIdentityId'] = "db9476fd-de77-4977-839f-4f943ff5d685"

        response = get_shared_list.get_shared_list_main(api_gateway_get_list_event)
        body = json.loads(response['body'])

        assert body['error'] == "List ID 12345678-abcd-abcd-123456789112 did not have a shared item with user db9476fd-de77-4977-839f-4f943ff5d685.", "Get list response did not contain the correct error message."
